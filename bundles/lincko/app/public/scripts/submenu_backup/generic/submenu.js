...
Submenu.prototype.Remove = function(){
	clona = this.wrapper.clone(); //This helps to delete all listeners on childs
	this.wrapper.replaceWith(clona);
	clona.remove();
	delete clona;
...
function Submenu(menu, next, param) {
	...
	this.prefix = false;
	this.attribute = null;
	function Constructor(Elem){console.log(Elem.Wrapper());
...
function submenu_Build(menu, next, hide){
	...
	//If the tab already exists, just close it if we launch again de action
	if(hide){
		for(var index in submenu_obj){
			if(submenu_obj[index].menu === menu){
