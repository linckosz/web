...
var burger_keyboard = function(elem, lineHeight, shortcuts, burgerData, fn_enter, project_id, dropdown_cb_custom){
	...
	*/
	...
	//if no initial data is given, _onKey for at or plus is turned true. If true, burgerData is cleared upon dropdown destroy cb
	that.burgerData_onKey = {
		at: false,
		plus: false,
	};
	that.burgerData = {};
	if(burgerData && typeof burgerData == 'object' && typeof burgerData.at == 'object'){ that.burgerData.at = burgerData.at; }
	else{ that.burgerData_onKey.at = true; }
	...
	that.dropdown_cb.cb_destroy = function(){
		...
		if(that.burgerData_onKey.at){
			delete that.burgerData.at;
		}
	   	...
	   	if(!that.dropdownInst 
			...
			if(latestChar == burger_shortcuts.at){
				...
				if(that.burgerData.at){
					that.burgerData.at = that.burger_data.at;
				}
				else if(that.project_id){
					that.burgerData.at = burger_list.in_charge('projects', that.project_id);
				}
				else{
...
var burger_dropdown = function(id, data, position, lineHeight, cb_create, cb_select, cb_destroy, focus_elem, flipped, extra){
	...
	that.elem = null;
	that.data = data;
	that.extra = typeof extra == 'object' ? extra : {};
...
burger_dropdown.prototype.build_elem_data = function(data){
	...
	//add headers
	if(that.extra.header){
		elem_dropdown_inner.append('<div>'+that.extra.header+'</div>');
	}

	var elem_option = $('#-burger_option').clone().removeAttr('id');//.addClass('burger_option_users');
