...
class SimpleImageCaptcha extends SimpleImage {
	...
	public function lincko_addcaptcha($cap_pic, $num, $total=4) {
		if($num>=1 && $num<=$total){
			...
			$degrees = rand(-20,20);
			$src_im->lincko_rotation($degrees); //Random rotte
			
			$sizeratio = rand(70,95)/100;
			$maxW = floor($sizeratio*$spaceW);
			$maxH = floor($sizeratio*$spaceH);
			$src_im->lincko_resizelimit($maxW,$maxH); //Random scale
	...
	public function lincko_resizelimit($width,$height) {
	...
	public function lincko_rotation($degrees) {
	...
	public function lincko_get_image(){
		return $this->image;
	}
